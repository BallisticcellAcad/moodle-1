...
M.mod_evoting.form_init = function (Y, boolUpdate) {
    ...
    $(function () {
        ...
        // Get idCourse
        var idCourse = $("#inputIdCourse").val();
        ...
        for (var i = 0; i < question_repeats; i++) {
            ...
            for (var j = 0; j < maxOptions; j++) {
                ...
                if (j < 4) {
                    $('#fitem_id_option' + j + '_' + i).addClass("visible");
                    $('#fitem_id_right' + j + '_' + i).addClass("visible");
                } else {
                    $('#fitem_id_option' + j + '_' + i).addClass("noVisible");
                    $('#fitem_id_right' + j + '_' + i).addClass("noVisible");

                    if (valueInput.length == 0) {
                        $('#fitem_id_option' + j + '_' + i).addClass("noVisible");
                        $('#fitem_id_option' + j + '_' + i).removeClass("visible");
                        $('#fitem_id_right' + j + '_' + i).addClass("noVisible");
                        $('#fitem_id_right' + j + '_' + i).removeClass("visible");
                    } else {
                        $('#fitem_id_option' + j + '_' + i).addClass("visible");
                        $('#fitem_id_option' + j + '_' + i).removeClass("noVisible");
                        $('#fitem_id_right' + j + '_' + i).addClass("visible");
                        $('#fitem_id_right' + j + '_' + i).removeClass("noVisible");
                    }
                }
        ...
        $(".addChoice").click(function () {
            ...
            var choiceToAdd = $('#fitem_id_option0' + '_' + numberQuestion).parent().nextAll()
                .find(".noVisible").first();
            var idChoice = choiceToAdd.prop("id");

            if(typeof idChoice === "undefined"){
                alert(M.util.get_string('noaddChoice','evoting'));
                return false;
            } else {
                var numberChoice = parseInt(idChoice.slice(15, idChoice.length - 2));
            }
            ...
            if (numberChoice != 0) {
                ...
                var rightToAdd = $('#fitem_id_right0' + '_' + numberQuestion).parent().nextAll().find(".noVisible").first();
                rightToAdd.addClass("visible");
                rightToAdd.removeClass("noVisible");
            } else {
                alert(M.util.get_string('noaddChoice','evoting'));
            }
        ...
        $(".deleteButton").click(function () {
            ...
            if (callbackDialog == true) {
                ...
                var numberQuestion = parseInt(idButtonDelete.charAt(idButtonDelete.length - 1));
                ...
                //Delete div Question
                $("#fitem_id_questionname" + "_" + numberQuestion).parent().remove();
                ...
                // Show only last delete button Question
                if (numberQuestion == 1) {
                    $("#fitem_id_deleteQuestion_" + (numberQuestion - 1)).hide();
                } else {
                    $("#fitem_id_deleteQuestion_" + (numberQuestion - 1)).show();
                }
                ...
                $('html, body').animate({scrollTop: $('#fitem_id_questionname_' + (numberQuestion - 1)).offset().top}, 500);
            }
        });
    });
