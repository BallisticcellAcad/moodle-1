...
/**
 * Helper functions for course_overview_gl block
 *
 * @package    block_course_overview_gl
 */

define('BLOCKS_COURSE_OVERVIEW_GL_SHOWCATEGORIES_NONE', '0');
define('BLOCKS_COURSE_OVERVIEW_GL_SHOWCATEGORIES_ONLY_PARENT_NAME', '1');
define('BLOCKS_COURSE_OVERVIEW_GL_SHOWCATEGORIES_FULL_PATH', '2');
 ...
 */
function block_course_overview_gl_get_overviews($courses) {
    $htmlarray = array();
...
/**
 * Sets user preference for maximum courses to be displayed in course_overview_gl block
 *
 * @param int $number maximum courses which should be visible
 */
function block_course_overview_gl_update_mynumber($number) {
    set_user_preference('course_overview_gl_number_of_courses', $number);
}

/**
 * Sets user course sorting preference in course_overview_gl block
 *
 * @param array $sortorder list of course ids
 */
function block_course_overview_gl_update_myorder($sortorder) {
    ...
    set_user_preference('course_overview_gl_course_sortorder', $value);
}

/**
 * Gets user course sorting preference in course_overview_gl block
 *
 * @return array list of course ids
 */
function block_course_overview_gl_get_myorder() {
    if ($value = get_user_preferences('course_overview_gl_course_sortorder')) {
        return explode(',', $value);
    }
    // If preference was not found, look in the old location and convert if found.
    $order = array();
    if ($value = get_user_preferences('course_overview_gl_course_order')) {
        $order = unserialize_array($value);
        block_course_overview_gl_update_myorder($order);
        unset_user_preference('course_overview_gl_course_order');
    }
    return $order;
}
 ...
 */
function block_course_overview_gl_get_child_shortnames($courseid) {
    global $DB;
    ...
    if ($results = $DB->get_records_sql($sql, $params)) {
        ...
        if ($total > 10) {
            ...
            if ($diff > 1) {
                $suffix = get_string('shortnamesufixprural', 'block_course_overview_gl', $diff);
            } else {
                $suffix = get_string('shortnamesufixsingular', 'block_course_overview_gl', $diff);
            }
        }
        $shortnames = get_string('shortnameprefix', 'block_course_overview_gl', implode('; ', $shortnames));
        $shortnames .= $suffix;
    }
...
/**
 * Returns maximum number of courses which will be displayed in course_overview_gl block
 *
 * @param bool $showallcourses if set true all courses will be visible.
 * @return int maximum number of courses
 */
function block_course_overview_gl_get_max_user_courses($showallcourses = false) {
    ...
    if (empty($config->forcedefaultmaxcourses)) {
        if ($showallcourses) {
            ...
            $limit = get_user_preferences('course_overview_gl_number_of_courses', $limit);
        }
    }
    return $limit;
}
 ...
 */
function block_course_overview_gl_get_sorted_courses($showallcourses = false) {
    global $USER, $DB;

    $limit = block_course_overview_gl_get_max_user_courses($showallcourses);
    ...
    foreach ($courses as $c) {
        if(!is_course_for_gl($c->category)) {
            unset($courses[$c->id]);
            continue;
        }
    ...
    $order = block_course_overview_gl_get_myorder();
...
function is_course_for_gl($category_id) {
    global $DB;
