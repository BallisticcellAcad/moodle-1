...
class theme_controller
{
        ...
        if ($frontpagecontenttype) { // Dynamic image slider.
            ...
            if (!$contenttype) {
                ...
                if ($staticimage == "" || $staticimage == null) {
                    $sliderdata['staticimage'] = \theme_remui\toolbox::pix_url('slide', 'theme');
                } else {
    ...
    public static function save_user_profile_info($fname, $lname, $emailid, $description, $cityname, $country, $userFields) {
        global $DB;
        $user = $DB->get_record('user', array('id' => $userFields->id));
    ...
    public static function get_recent_blog($start = 0, $blogcount = 10) {
        ...
        foreach ($blogentries as $blogentry) {
            ...
            if (!$attrs) {
                $blog[$blogentry->id]['imagesrc'] = \theme_remui\toolbox::pix_url('400x300', 'theme');
