    ...
    return {
        initialise: function ($params) {
            ...
            $('#page-user-profile .form-horizontal #btn-save-changes').click(function () {
                ...
                var gander = $('#page-user-profile .form-horizontal #select-gander option:selected').text();
                var birthdate = $('#inputBirthdate').val();
                var interests = [];
                $('span[role="listitem"]').each(function (index, item) {
...
//                    url: M.cfg.wwwroot + '/theme/remui/rest.php?action=save_user_profile_settings&fname=' + fname + '&lname=' + lname + '&emailid=' + emailid + '&description=' + description + '&city=' + city + '&country=' + country,
			...
			$.each(data, function (index, municipality) {
				newData.push(municipality);
			});
			newData.sort(compare);
                        $.each(newData, function (index, municipality) {
			...
			newData.sort(compare);
                        $.each(newData , function (index, city) {
			...
			$.each(data, function(index, school){
				newData.push(school);
                        ...
                        $.each(newData, function (index, school) {
            ...
            $('#page-user-profile .form-horizontal #form_autocomplete_input').change(function () {
                var tag = this.value;
                if (tag.length > 0) {
                    $('#noselection').remove();
                    var newTag = $('<span></span>');
                    newTag.attr("role", "listitem");
                    newTag.attr("data-value", tag);
                    newTag.attr("aria-selected", "true");
                    newTag.addClass('label label-info');
                    newTag.append('<span aria-hidden="true">× </span>');
                    newTag.append(tag);                    
                    $('#form_autocomplete_selection').append(newTag);
                }
            });

            $('#page-user-profile .form-horizontal #form_autocomplete_selection')
                    .on('click', 'span[role="listitem"]', function () {
                $(this).remove();
                var remainingInterests = $('span[role="listitem"]');
                        if (remainingInterests.length === 0) {
                            var noselection = $('<span></span>');
                            noselection.attr('id', 'noselection');
                            noselection.text(M.util.get_string('noselection', 'theme_remui'));
                            $('#form_autocomplete_selection').append(noselection);
                        }
            });
